...
local win = library:Window("Selet",[[Hub]],[[Normal]],"6022668898",Enum.KeyCode.RightControl)
local Main3 = win:Tab("Main",[[7040391851]])
local Main = win:Tab("Item Farm",[[7251993295]])

Main3:Seperator("Welcome To Selet Hub")

local Time = Main3:Label("Executor Time");spawn(function() getgenv().Time = true;while true do wait(.1) UpdateTime() end end);function UpdateTime() local date = os.date("*t");local hour = (date.hour) % 24;local ampm = hour < 12 and "AM" or "PM";local timezone = string.format("%02i:%02i:%02i %s", ((hour -1) % 12) + 1, date.min, date.sec, ampm);local datetime = string.format("%02d/%02d/%04d", date.day, date.month, date.year);local LocalizationService = game:GetService("LocalizationService");local Players = game:GetService("Players");local player = Players.LocalPlayer;local name = player.Name;local result, code = pcall(function()   return LocalizationService:GetCountryRegionForPlayerAsync(player)  end);Time:Set(" : " .. timezone);Time:Set("Executor Time: " .. datetime .. " [ " .. code .. " ]");spawn(function() if getgenv().Time then pcall(function()  while wait() do  Time()  end end) end end) end

Time = Main3:Label("Server Time")
...
Main3:Seperator("Main Farm")

        Main3:Toggle("Auto Second Sea","6022668898",_G.AutoSecondSea,function(value)
    ...
    if World2 then
        Main3:Toggle("Auto Third Sea","6022668898",_G.AutoThirdSea,function(value)
...
Main3:Toggle("Auto Farm","6022668898",_G.AutoFarm,function(value)
    _G.AutoFarm = value
    _G.Set = value
                                    ...
                                    if v.Name == Mon then
                                        if string.find(game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text, NameMon) then
                                            repeat task.wait()
                                                ...
                                                Wait(1)
                                                topos(v.HumanoidRootPart.CFrame * CFrame.new(-20,30,0))
                                                Wait(2)
                                                topos(v.HumanoidRootPart.CFrame * CFrame.new(0,30,20))
                                                Wait(1)
                                                topos(v.HumanoidRootPart.CFrame * CFrame.new(20,30,0))
    ...
    local SelectWeapona = Main3:Dropdown("Select Weapon / Combat",WeaponList,function(value)
...
Main3:Toggle("Auto SetSpawn Point","6022668898",true,function(x)
    _G.Set = x
end)
...
spawn(function()
   ...
   Main3:Seperator("Setting Farm")
  
   Main3:Toggle("FastAttack","6022668898",true,function(value)
	getgenv().FastAttack = value
end)

Main3:Toggle("Very Fast Attack","6022668898",false,function(value)
        getgenv().FastAttack6 = value
...
Main3:Toggle("Fast Attack (Used To Auto Farm)","6022668898",false,function(value)
    _G.SuperFastAttack = value
end)

Main3:Toggle("Auto Attack","6022668898",false,function(value)
...
game:GetService("RunService").RenderStepped:Connect(function()
   ...
   Main3:Toggle("Bring Mob","6022668898",true,function(Mag)
    _G.BringMonster = Mag
    end)
    spawn(function()
        while task.wait() do
    ...
    Main3:Toggle("Enable/Disable Nofication","6022668898",getgenv().n,function(value)
    getgenv().n = value
end)

Main3:Toggle("White Screen","6022668898",false,function(value)
	getgenv().W = value
end)
...
Main3:Button("Remove Animation + Remove Sound Attack", function()
    ...
    Main3:Toggle("Remove Attack","6022668898",false,function(value)
...
spawn(function()
    ...
    Main3:Toggle("Delete Attack FIx","6022668898",nil,function(d)
    _G.DeleteAttackFx = d
...
Main3:Slider("Select Level Lock Farm",1,LockLevelValue,LockLevelValue,nil,function(value)
	LockLevelValue = value
end)
Main3:Toggle("Lock Level Farm","6022668898",locklevel,function(value)
	LockLevel = value
end)
	spawn(function()
		while wait(.1) do
...
getgenv().FastAttack = true --[[ false = เธเธดเธ” / true = เน€เธเธดเธ” ]]
...
spawn(function()
    ...
    Main3:Seperator("Misc Setting Mastery")
